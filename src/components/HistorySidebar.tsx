...
import { History, Clock, Palette, Square, Circle, RotateCcw, Trash2, Sparkles } from "lucide-react"
...
export function HistorySidebar({ sessionId, onLoadHistory, isOpen, onToggle }: HistorySidebarProps) {
  ...
  return (
    <div className={`fixed top-4 left-4 z-40 transition-all duration-300 ease-in-out ${isOpen ? "w-88" : "w-12"}`}>
      {/* Toggle Button */}
      <Button
        onClick={onToggle}
        variant="outline"
        size="sm"
        className={`absolute top-4 bg-white shadow-lg hover:shadow-xl transition-all duration-200 z-50 ${isOpen ? "right-4" : "right-2"
          }`}
        title={isOpen ? "Hide History" : "Show History"}
      >
        <History className={`h-4 w-4 transition-transform duration-200 ${isOpen ? "rotate-0" : "rotate-12"}`} />
        {isOpen && <span className="ml-2 text-xs">Hide</span>}
      </Button>

      {/* Sidebar Content */}
      <div
        className={`h-[calc(100vh-2rem)] transition-all duration-300 ease-in-out ${isOpen ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4 pointer-events-none"
          }`}
      >
        {!sessionId ? (
                  ...
                  {isLoading ? (
                    ...
                    <div className="space-y-3">
                      {history.map((item, index) => (
                        <div key={item.id}>
                          <div
                            className="group relative p-4 rounded-lg border border-border/50 hover:border-indigo-200 hover:bg-gradient-to-r hover:from-indigo-50/50 hover:to-purple-50/50 cursor-pointer transition-all duration-200 hover:shadow-md"
                          ...
                          >
                            ...
                            <div className="flex items-center justify-between mb-3">
                              ...
                              <Button
                                variant="ghost"
                                size="sm"
                                className="opacity-0 group-hover:opacity-100 h-7 w-7 p-0 hover:bg-indigo-100 hover:text-indigo-700 transition-all duration-200"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  onLoadHistory(item)
